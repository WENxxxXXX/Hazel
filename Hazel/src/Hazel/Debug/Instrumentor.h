...
namespace Hazel {
    ...
    public:
        ...
        // 会话用于较高层次的函数(In Entry Point),宏用于较低层次的函数（比如Sandbox 中的绘制操作）
        ...
        {            
            ...
            if (m_CurrentSession) 
            {
                ...
                { 
                    HZ_CORE_ERROR("Instrumentor::BeginSession('{0}') is creating a new session when session '{1}' already open.", 
            ...
            if (m_OutputStream.is_open()) 
            {
                ...
                // Edge case : BeginSession() might be before Log::Init() !!!
                if (Log::GetCoreLogger()) { HZ_CORE_ERROR("Instrumentor can't open result file '{0}'.", filepath); }
            }
        }
    ...
    public:
        ...
        void Stop()
        {
            ...
            Instrumentor::Get().WriteProfile({ m_Name, highResStart, elapsedTime, std::this_thread::get_id() });                           // In "Instrumentor"
