...
namespace Hazel
{
	...
	//声明这个函数是来自application.cpp中的函数，并设为全局函数
	// (这样就不用在这个文件中包含glad/glad.h， 不然会导致Sandbox出现问题)
	...
	struct LayoutElement {
		...
		LayoutElement(ShaderDataType type, const std::string& name, bool normalized = false)
			...
			GLType(Hazel::GetTypeToGLType(type))
	...
	public:
		...
		//方便在 for each 中将 m_Element 作为 range
		...
		//如果 layout 是 const，则需要 const 类型的迭代器协助 layout 作为 range
