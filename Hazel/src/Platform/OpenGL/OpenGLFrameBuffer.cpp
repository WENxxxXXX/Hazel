...
namespace Utils
{
	...
	static bool IsDepthFormat(Hazel::FrameBufferAttachmentFormat format)
	{
		switch (format)
		{
		case Hazel::FrameBufferAttachmentFormat::DEPTH24STENCIL8: return true;
		}
		return false;
	}

	static GLenum HazelTexFormatToGL(Hazel::FrameBufferAttachmentFormat format)
	{
		switch (format)
		{
...
namespace Hazel
{
	...
	int OpenGLFrameBuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
	{
		HZ_CORE_ASSERT((attachmentIndex < m_ColorAttachmentIDs.size()), 
			"Attachment index is beyond the scope of Attachments which we set");
