...
namespace Hazel
{
	...
	void ContentBrowserPanel::OnImGuiRender()
	{
		...
		// m_CurrentDirectory 中的每一个目录项：DirectoryEntry
		for (auto& directoryEntry : std::filesystem::directory_iterator(m_CurrentDirectory))
		{
			...
			if (ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
			{
				...
				if (directoryEntry.is_regular_file())
				{
					...
					//"D:\\1-toolbox\\Cherno-GameEngine\\Hazel" / path;

					//// 使用 ShellExecute 打开VSCODEs
					//ShellExecuteA(nullptr, "open", 
					//	"C:\\Users\\yewen\\AppData\\Local\\Programs\\Microsoft VS Code\\Code.exe", 
					//	absolutePath.string().c_str(), nullptr, SW_SHOWNORMAL);
				}
			}
