...
namespace Hazel
{
		...
		DrawComponent<CameraComponent>("Camera", entity, [this](auto& component)
			{
				...
				if (ImGui::BeginCombo("Projection", currentProjectionType))			// Combo box preview value needs to be a c_str
				{
					...
					for (int i = 0; i < 2; i++)
					{
						...
						if (isSelected)
							ImGui::SetItemDefaultFocus();	// ���ڸ��½��㣨���㲻ͬ�ڸ�����ʾ��
					}
					ImGui::EndCombo();
				}
	...
	static void SceneHierarchyPanel::DrawComponent(const std::string& name, Entity& entity, UIFunction uiFunc)
	{
		...
		if (entity.HasComponent<T>())
		{
			...
			// We need to get the correct size of the available content region, 
			// which is in the properties window, not in the tree node drawn in this window
			ImVec2 contentRegionAvail = ImGui::GetContentRegionAvail();
			
			ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2{ 4,4 });
			// We need to get the correct size of the button, which is set in the StyleVar scope
